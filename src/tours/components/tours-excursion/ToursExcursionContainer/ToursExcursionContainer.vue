...
import { fetchTourExcursion } from "@/tours/services";
import type { FetchTourExcursionQuery } from "@/tours/services";
...
const query = useQuery<FetchTourExcursionQuery>();

const { data, pending } = useLazyAsyncData("tours-excursion-id", () =>
    fetchTourExcursion(props.id, query.value)
);
</script>
